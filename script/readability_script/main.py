...
def main():
    ...
    for filename in os.listdir('D:\Desktop\Redability\data\pipeline'):
        print("n° " + filename)
        write_csv('../../data/pipeline/' + str(filename) )
...
def init_csv():
    ...
    with open('../../data/dataset/dataset.csv', 'w', encoding="utf8") as f:
        writer = csv.writer(f)
        writer.writerow(header)


def write_csv(file_name):
    row_data = [file_name.split("/",maxsplit=2)[2], lines_of_codes(file_name), avg_assignment(file_name), avg_blank_lines(file_name),
                avg_colon(file_name),
    ...
    with open('../../data/dataset/dataset.csv', 'a+', encoding="utf8") as f:
        writer = csv.writer(f)
