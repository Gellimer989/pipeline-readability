...
def init_csv():
    ...
    with open('dataset/parentCommit.csv', 'w', encoding="utf8") as f:
        writer = csv.writer(f)
...
def get_pipeline(sha_commit, project_name):
    if sha_commit is not None:
        ...
        if r.text != "404: Not Found":
            with open('pipeline/' + str(sha_commit) + '.txt', 'w', encoding='utf-8') as f:
                f.write(r.text)
...
def write_on_csv(id, repo_name, hash, redable, commit_message):
    ...
    with open('dataset/parentCommit.csv', 'a', encoding="utf8") as f:
        writer = csv.writer(f)
        writer.writerow(row_data)


def main():
    df_commit = pd.read_csv('repo_csv/ListOfCandidateCommitsForRQ1.csv')
    ...
    df_pipeline = pd.read_csv('dataset/parentCommit.csv')
